...
// The single place a binary's digest is matched against the allowlist.
//
// Deliberately shared rather than reimplemented per caller: the wasm engine
// uses it to decide what to grant at construction, and the transform layer
// uses it to notice that a running module's grant changed. Two lookups that
// could drift would mean a module keeping a capability the allowlist no
// longer gives it, which is the failure this function exists to prevent.
//
// Returns nullptr for an empty digest - a binary with no recorded digest can
// never be trusted, since there is nothing to match.

...
ss::sstring wasm_trusted_module::validate() const {
    ...
    if (
      has_capability(wasm_capability::relay_consumer)
      && !has_capability(wasm_capability::shared_memory)) {
        // Same class of mistake as 'network' with no targets, and rejected for
        // the same reason: the relay delivers through
        // engine::write_shared_memory, so this grant has nowhere to put the
        // data it subscribes to. Accepting it silently would leave an operator
        // believing a guest is receiving relay data when it cannot.
        return "the 'relay_consumer' capability requires 'shared_memory' - "
               "the relay delivers into the guest's shared-memory region, so "
               "relay_consumer alone has nowhere to deliver to";
    }
    return {};
}

const wasm_trusted_module* find_trusted_module(
  const std::vector<wasm_trusted_module>& trusted,
  std::string_view sha256_hex) {
    if (sha256_hex.empty()) {
        return nullptr;
    }
    auto it = std::ranges::find(
      trusted, sha256_hex, &wasm_trusted_module::sha256_hex);

...
ss::future<int32_t> network_module::recv(
  int32_t handle, ffi::array<uint8_t> buf, uint32_t* out_len) {
    ...
    // Flush whatever the guest has queued before blocking on a read. send()
    // only enqueues, and the per-batch drain in the engine's transform loop
    // runs AFTER every record in the batch - so without this a guest that
    // sends a request and then waits for the reply deadlocks against a peer
    // that is waiting for that request, and nothing ever times out. That is
    // not an exotic sequence: it is what a request/response exchange looks
    // like, and the guest's own main() - where there is no batch to end -
    // cannot reach the batch drain at all.
    //
    // Asking to read is what makes this safe to do here: a guest that wants
    // a reply has, by asking, declared its request complete. Still
    // non-blocking - dispatch_push() is fire-and-forget under the push gate,
    // and the read below is the suspension point that lets it run.

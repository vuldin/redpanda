      ...
      [timeout,
       batches = chunked_vector<model::record_batch>(
        ...
        -> ss::future<result<model::offset, cluster::errc>> {
          // Take ownership of the captured batches into the coroutine frame
          // BEFORE the first suspension, and iterate that.
          //
          // This lambda is a coroutine whose closure is shipped to another
          // shard, and a lambda coroutine's captures live in the closure
          // object, not the coroutine frame - so the closure can be moved or
          // destroyed while the coroutine is suspended. Iterating the capture
          // across the co_await below would hold an iterator into a container
          // that gets moved underneath it, and seastar::chunked_vector's move
          // calls update_generation() on both sides, so the next ++it trips
          // check_generation() and aborts the process. Indexing instead of
          // iterating would not be enough: the closure itself may be gone,
          // not merely generation-bumped.
          auto owned = std::move(batches);
          result<model::offset, cluster::errc> last_result
            = cluster::errc::success;
          for (auto& batch : owned) {

...
ss::future<> processor::run_consumer_loop(kafka::offset offset) {
    ...
    while (!_as.abort_requested()) {
        ...
        // transform's read config is a NO-OP.
        // `storage::local_log_reader_config` has no min_bytes field at all - it
        // exists only on the kafka-level and cloud reader configs - and the
        // accumulate/long-poll logic lives in `kafka/server/handlers/fetch.cc`
        // (`over_min_bytes`), a path transforms never take. A knob wired to
        // min_bytes would have measured as "batching does not help".

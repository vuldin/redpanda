...
private:
    // Deliver to subscribers for ntp on THIS shard. Called both directly by
    // push() for the local case and on a peer instance by the cross-shard
    // dispatch, which is why it is a member rather than a free function.
    void deliver_locally(const model::ntp& ntp, const iobuf& data);

    absl::flat_hash_map<model::ntp, absl::flat_hash_map<uint32_t, subscription*>>
      _by_ntp;
    // Which OTHER shards have a subscriber for this ntp. push() consults
    // this to decide whether a cross-shard dispatch is needed at all, so
    // the no-remote-subscriber case costs one hash lookup and no hop.

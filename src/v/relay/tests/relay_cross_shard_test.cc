...
public:
    ...
    static void set_stage_metrics(bool v) {
        ss::smp::invoke_on_all([v] {
            ::config::shard_local_cfg().relay_stage_metrics_enabled.set_value(
              v);
        }).get();
    }
...
TEST_F(relay_cross_shard_fixture, PushFromOneShardReachesSubscriberOnAnother) {
    ...
    tests::cooperative_spin_wait_with_timeout(
      ...
      [this, ntp] {
          ...
          return svcs.invoke_on(1, [](service&) { return g_sub.calls > 0; });
      })
      .get();
    ...
    svcs.invoke_on(1, [](service& s) { s.remove_subscription(*g_sub_id); })
      .get();
}
...
TEST_F(relay_cross_shard_fixture, PushReachesSubscribersOnEveryRemoteShard) {
    ...
    for (unsigned sh = 1; sh < shards; ++sh) {
        svcs
          .invoke_on(
            sh,
            [ntp](service& s) { g_sub_id = s.add_subscription(ntp, &g_sub); })
          .get();
    }
    ...
    tests::cooperative_spin_wait_with_timeout(
      ...
      [this, ntp, shards] {
          ...
          return svcs.invoke_on(
            shards - 1, [](service&) { return g_sub.calls > 0; });
      })
      .get();
    ...
    for (unsigned sh = 1; sh < shards; ++sh) {
        ...
        EXPECT_EQ(last_size, payload)
          << "shard " << sh
          << " saw the wrong payload size, which is what a "
...
TEST_F(relay_cross_shard_fixture, RemovedSubscriberStopsReceivingAcrossShards) {
    auto ntp = test_ntp();
    svcs
      .invoke_on(
        1, [ntp](service& s) { g_sub_id = s.add_subscription(ntp, &g_sub); })
    ...
    auto calls_before
      = svcs.invoke_on(1, [](service&) { return g_sub.calls; }).get();
    svcs.local().push(ntp, make_data(16));
